        ...
        for _, line in tqdm(enumerate(fc), desc='process corpus file'):
            ...
            for i in range(len(sentences)):
                ...
                for fb_id in sent_ids:
                    ...
                    # TODO -- If don't wanna extend the sentence by pre and post sentence, directly comment this part
                    ...
                    record = {"s": sent_left + sent_right,  # direct concat left and right part
...
def convert_to_index(data, vocab, entity_names, save_path, name):
    ...
    for record in tqdm(data, desc='convert {} dataset to index'.format(name)):
        record_idx = {"s": [vocab[word] if word in vocab else vocab[UNK] for word in record['s']],
