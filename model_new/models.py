...
from model.nns import bidirectional_dynamic_rnn, dynamic_rnn
...
class Model(object):
    ...
    def _build_model_op(self):
        ...
        with tf.variable_scope('lexical_encoder'):
            lexical_rnn = dynamic_rnn(self.config.num_units * 2, use_peepholes=True, scope='lexical_rnn')
        ...
        with tf.variable_scope('context_encoder'):
            context_rnn = dynamic_rnn(self.config.num_units * 2, use_peepholes=True, scope='context_rnn')
        ...
        with tf.variable_scope('project'):
            ...
            # self.logits = tf.reshape(tf.one_hot(tf.nn.top_k(self.logits).indices, 3), shape=[-1, 3])
